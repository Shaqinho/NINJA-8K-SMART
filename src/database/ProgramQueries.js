...
export const insertLiveCategories = async (categories) => {
  ...
  await db.execute('DELETE FROM live_categories');
  
  for (const cat of categories) {
    await db.run(
      'INSERT OR REPLACE INTO live_categories (category_id, category_name, parent_id) VALUES (?, ?, ?)',
      [cat.category_id, cat.category_name, cat.parent_id || 0]
    );
  }
  
  console.log(`✅ Inserted ${categories.length} live categories`);
};
...
export const insertVodCategories = async (categories) => {
  ...
  console.log(`✅ Inserted ${categories.length} vod categories`);
};
...
export const insertSeriesCategories = async (categories) => {
  ...
  await db.execute('DELETE FROM series_categories');
  ...
  console.log(`✅ Inserted ${categories.length} series categories`);
};

// ============================================================================
// INSERT ITEMS (VOD, SERIES)
// ============================================================================

/**
 * Insert VOD Items (Movies)
 */
export const insertVodItems = async (items) => {
  ...
  console.log(`✅ Inserted ${items.length} vod items`);
};
...
export const insertSeriesItems = async (items) => {
  ...
  await db.execute('DELETE FROM series_items');
  
  for (const item of items) {
    await db.run(
      `INSERT OR REPLACE INTO series_items 
       (series_id, name, category_id, category_name, cover, rating, year, genre) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
  ...
  console.log(`✅ Inserted ${items.length} series items`);
};
...
/**
 * Get Live Categories from DB
 */
export const getLiveCategoriesFromDB = async () => {
  const db = getDatabase();
  return await db.query('SELECT * FROM live_categories ORDER BY category_name');
};

/**
 * Get VOD Categories from DB
 */
export const getVodCategoriesFromDB = async () => {
  const db = getDatabase();
  return await db.query('SELECT * FROM vod_categories ORDER BY category_name');
};

/**
 * Get Series Categories from DB
 */
export const getSeriesCategoriesFromDB = async () => {
  const db = getDatabase();
  return await db.query('SELECT * FROM series_categories ORDER BY category_name');
};
...
/**
 * Get VOD Items from DB
 */
export const getVodItemsFromDB = async (categoryId = null) => {
  const db = getDatabase();
  
  if (categoryId) {
    return await db.query(
      'SELECT * FROM vod_items WHERE category_id = ? ORDER BY name',
      [categoryId]
    );
  }
  
  return await db.query('SELECT * FROM vod_items ORDER BY name');
};

/**
 * Get Series Items from DB
 */
export const getSeriesItemsFromDB = async (categoryId = null) => {
  const db = getDatabase();
  
  if (categoryId) {
    return await db.query(
      'SELECT * FROM series_items WHERE category_id = ? ORDER BY name',
      [categoryId]
    );
  }
  
  return await db.query('SELECT * FROM series_items ORDER BY name');
};
...
const ProgramQueriesExports = { 
  ...
  insertVodItems,
  insertSeriesItems,
  getLiveCategoriesFromDB,
  getVodCategoriesFromDB,
  getSeriesCategoriesFromDB,
  getVodItemsFromDB,
  getSeriesItemsFromDB,
